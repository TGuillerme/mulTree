...
test_that("clean.tree.table works", {
    # Errors
    expect_error(clean.tree.table(TRUE))
    # Output is a list...
    expect_is(test, "list")
    # ... of 4 elements.
...
test_that("clean.data works", {
    ...
    # ... of 4 elements.
    expect_equal(length(cleaned), 4)
    # First element are trees...
    expect_is(cleaned[[1]], "multiPhylo")
    # ...with 4 taxa each.
    expect_equal(unique(unlist(lapply(cleaned[[1]], Ntip))), 4)
    # Second element is a table...
    expect_is(cleaned[[2]], "data.frame")
    # ...with four rows.
    expect_equal(nrow(cleaned[[2]]), 4)
    # Third element contains "F"
    expect_equal(cleaned[[3]], "F")
    # Forth element contains "E"
    expect_equal(cleaned[[4]], "E")
})
