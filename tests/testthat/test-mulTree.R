...
test_that("read.key works - scan option deactivated!", {
    #Message ()
    expect_message(read.key("message1", scan=FALSE))
    expect_message(read.key("message1", "message2", scan=FALSE))
    #Too many messages
    expect_error(read.key("message1", "message2", "message3", scan=FALSE))
    #Not enough messages
    expect_error(read.key(scan=FALSE))
})
...
test_that("lapply.MCMCglmm works", {
    ...
    expect_error(lapply.MCMCglmm(tree[[1]], mulTree.data, formula, priors, parameters), warn=FALSE)
    # mulTree.data is not mulTree
    expect_error(lapply.MCMCglmm(1, data, formula, priors, parameters), warn=FALSE)
    # formula is not a formula
    expect_error(lapply.MCMCglmm(1, mulTree.data, formula="bla", priors, parameters), warn=FALSE)
    # priors' not a list
    expect_error(lapply.MCMCglmm(1, mulTree.data, formula, prior=1, parameters), warn=FALSE)
    # parameters is not a vector
    expect_error(lapply.MCMCglmm(1, mulTree.data, formula, priors, parameters=1), warn=FALSE)
    # wrong additional argument
    expect_error(lapply.MCMCglmm(1, mulTree.data, formula, priors, parameters, some_arg="whatever"), warn=FALSE)
    
    # When no errors, outputs a MCMCglmm object
    set.seed(1)
    test <- lapply.MCMCglmm(1, mulTree.data, formula, priors, parameters, warn=FALSE)
    # Output is MCMCglmm
    expect_is(test, "MCMCglmm")
    # MCMCglmm is of standard length
    expect_equal(length(test), 18)
})
...
test_that("convergence.test works", {
    # Errors
    # Not a list
    expect_error(convergence.test(model_tree1_chain1))
    # Not enough chains
    expect_error(convergence.test(list(model_tree1_chain1)))
    # Missing arguments (error + message)
    expect_error(expect_message(convergence.test("bla")))
    # Convergence works
    test <- convergence.test(list(model_tree1_chain1, model_tree1_chain2))
    # Output is gelman.diag
    expect_is(test, "gelman.diag")
    # gelman.diag is standard format
    expect_equal(length(test), 2)
    # psrf is standard matrix
    expect_is(test$psrf, "matrix")
})
...
test_that("ESS.lapply works", {
    # Errors
    # Something silly
    expect_error(ESS.lapply(3, 1))
    # ESS is a numeric vector (length 2)
    expect_is(ESS.lapply(model_tree1_chain1), "numeric")
    expect_equal(names(ESS.lapply(model_tree1_chain1)), c("(Intercept)","var2"))
})
...
test_that("Quick mulTree example works", {
    ...
    expect_error(mulTree(mulTree.data, formula = something ~ whatever, parameters = c(10000, 10, 1000), chains = 2, prior = priors, output = "quick_example", convergence = 1.1, ESS = 100))
    ...
    expect_error(mulTree(mulTree.data, formula = var1 ~ var2, parameters = c(10000, 10, 1000), chains = "plenty", prior = priors, output = "quick_example", convergence = 1.1, ESS = 100))
